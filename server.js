...
async function initDB() {
  ...
  await dbRun(`CREATE TABLE IF NOT EXISTS people (
    id TEXT PRIMARY KEY,
...
app.post('/api/groups', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    const id = uid();
    await dbRun(
      'INSERT INTO groups (id, owner_id, name, days) VALUES (?, ?, ?, ?)',
...
app.put('/api/groups/:id', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { name, days } = req.body;
    await dbRun(
      'UPDATE groups SET name = ?, days = ? WHERE id = ?',
...
app.post('/api/groups/:groupId/people', authMiddleware, async (req, res) => {
  try {
    ...
    const countRow = await dbGet(
      'SELECT COUNT(*) as c FROM people WHERE group_id = ?',
      [req.params.groupId]
    );
    const id = uid();
    await dbRun(
...
app.delete('/api/groups/:groupId/people/:id', authMiddleware, async (req, res) => {
  try {
    await dbRun(
      'DELETE FROM people WHERE id = ? AND group_id = ?',
...
app.put('/api/groups/:groupId/people/reorder', authMiddleware, async (req, res) => {
  try {
    ...
    for (let i = 0; i < order.length; i++) {
      await dbRun('UPDATE people SET order_index = ? WHERE id = ?', [i, order[i]]);
    }
    res.json({ ok: true });
  } catch(e) {
    res.status(500).json({ error: e.message });
...
app.post('/api/schedules/:groupId/:year/:semester', authMiddleware, async (req, res) => {
  try {
    ...
    for (const [dateKey, personName] of Object.entries(schedule)) {
      await dbRun(
